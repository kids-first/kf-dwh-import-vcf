...
object VariantCentricOutput {
  ...
  case class InternalFrequencies(
      upper_bound_kf: Freq = Freq(30, 12, 0.4, 9, 7),
      lower_bound_kf: Freq = Freq(27, 12, 0.4444444444, 9, 7)
  )

  case class Frequencies(
      one_thousand_genomes: OneThousandGenomesFreq = OneThousandGenomesFreq(),
      topmed: Freq = Freq(),
  ...
  case class CLINVAR(
      `clinvar_id`: String = "257668",
      `clin_sig`: List[String] = List("Benign"),
      `conditions`: List[String] =
        List("Congenital myasthenic syndrome 12", "not specified", "not provided"),
  ...
  case class GENES(
      `symbol`: Option[String] = Some("SCN2A"),
      ...
      `alias`: Option[List[String]] = Some(
        List("BII", "CACH6", "CACNL1A6", "Cav2.3", "EIEE69", "gm139")
      ),
