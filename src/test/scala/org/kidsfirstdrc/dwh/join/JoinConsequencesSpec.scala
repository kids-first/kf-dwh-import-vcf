...
class JoinConsequencesSpec
  ...
  "build" should "return a dataframe with all expected columns" in {
    withOutputFolder("output") { outputDir =>
      ...
      val existingCsq1 =
        JoinConsequenceOutput(study_ids = List(studyId3), release_id = "RE_PREVIOUS")
      ...
      Seq(ImportScores.Output())
        .toDF()
        .write
        .mode(SaveMode.Overwrite)
        .option("path", s"$outputDir/dbnsfp_original")
        .format("parquet")
        .saveAsTable("dbnsfp_original")

      When("Join Consequences")
      new JoinConsequences(Seq(studyId1, studyId2), releaseId, mergeWithExisting = true, "variant")
        .run()
      ...
      val expectedOutput = Seq(
        ...
        JoinConsequenceOutput(
          ...
          "protein_coding",
          "SNV",
          Some(Exon(Some(7), Some(27))),
          None,
          "ENST00000283256.10:c.781G>A",
          "ENSP00000283256.6:p.Val261Met",
          Some("chr2:g.166166916G>A"),
          Some(781),
          937,
          261,
          RefAlt("V", "M"),
