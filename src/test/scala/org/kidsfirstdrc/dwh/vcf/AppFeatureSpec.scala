  ...
  Feature("Run") {
    Scenario("Transform vcf into parquet files") {
      ...
      withOutputFolder("output") { output =>
        ...
        loadTestClinicalTable(
          "family_relationships",
          s"${DataService.family_relationships.rootPath}/Dataservice"
        )
        ...
        val occurrences =
          spark.table("variant.occurrences_sd_123456_re_abcdef").as[OccurrenceOutput]
        ...
        And(
          "Table variants_sd_123456_re_abcdef should contain rows for the given study and release"
        )
        val variants = spark
          .table("variant.variants_sd_123456_re_abcdef")
          .select(
            "chromosome",
            "start",
            "end",
        ...
        And(
          "Table consequences_sd_123456_re_abcdef should contain rows for the given study and release"
        )
        val consequences = spark
          .table("variant.consequences_sd_123456_re_abcdef")
          .select(
            "chromosome",
            "start",
            "end",
