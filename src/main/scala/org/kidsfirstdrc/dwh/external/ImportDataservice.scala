...
object ImportDataservice extends App {
  ...
  implicit val spark: SparkSession = SparkSession.builder
    ...
    .enableHiveSupport()
    .appName("Import Dataservice").getOrCreate()
  ...
  def build(studyIds: Set[String], releaseId: String, input: String, output: String, mergeExisting: Boolean, tables: Set[String]): Unit = {
    ...
    if (tables.contains("biospecimens")) {
      writeLoad(studyIds, releaseId, output, mergeExisting, "biospecimens") { (studyId, spark) =>
        ...
        val df = bio.join(part, bio("participant_id") === part("kf_id")).select(bio("*"), part("family_id") as "family_id")
        df
  ...
  def write(studyIds: Set[String], releaseId: String, input: String, output: String, mergeExisting: Boolean, name: String)(implicit spark: SparkSession): Unit = {
    writeLoad(studyIds, releaseId, output, mergeExisting, name) {
      (studyId, spark) => spark.read.parquet(s"$input/$name/$studyId")
    }
  }

  def writeLoad(studyIds: Set[String], releaseId: String, output: String, mergeExisting: Boolean, name: String)(load: (String, SparkSession) => DataFrame)(implicit spark: SparkSession): Unit = {
    import spark.implicits._
