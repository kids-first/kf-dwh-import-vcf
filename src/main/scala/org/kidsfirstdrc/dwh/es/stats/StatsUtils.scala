...
object StatsUtils {
  ...
  /** Fetch all Occurrences tables in a specific database that do not contain a release number
    ...
    */
  def getOccurrencesTableWORelease(
      database: String
  )(implicit spark: SparkSession): Array[String] = {
    import spark.implicits._

    spark
      .sql(s"show tables in $database")
      .where(
        $"tableName" like "occurrences_sd_%" and not($"tableName" like "occurrences_sd_%_re_%")
      )
      .select("tableName")
      .collect()
      .flatMap(_.toSeq)
  ...
  def getUnionOfOccurrences(database: String, tabledList: Array[String])(implicit
      spark: SparkSession
  ): DataFrame = {
