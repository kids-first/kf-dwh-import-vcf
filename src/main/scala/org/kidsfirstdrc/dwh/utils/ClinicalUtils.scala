...
object ClinicalUtils {
  ...
  implicit class DataFrameOps(df: DataFrame) {
    ...
    def joinAndMerge(
      ...
      df.joinByLocus(other, joinType)
        .withColumn(
          outputColumnName,
          when(col(otherFields.columns.head).isNotNull, struct(otherFields("*")))
            .otherwise(lit(null))
        )
    ...
    def selectLocus(cols: Column*): DataFrame = {
      val allCols =
        col("chromosome") :: col("start") :: col("reference") :: col("alternate") :: cols.toList
      df.select(allCols: _*)
    }

  }

  def loadClinicalTable(studyId: String, releaseId: String, tableName: String)(implicit
      spark: SparkSession
  ): DataFrame = {
  ...
  def getBiospecimens(studyId: String, releaseId: String, biospecimenIdColumn: String)(implicit
      spark: SparkSession
  ): DataFrame = {
    ...
    val all = b
      .join(p, b("participant_id") === p("kf_id"))
      .select("b.*", "p.is_proband", "p.affected_status")
